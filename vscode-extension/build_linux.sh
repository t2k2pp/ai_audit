#!/bin/bash
# =============================================================================
# ai_audit Linux用 VSIXビルドスクリプト
#
# 前提:
#   - Python 3.10以上がインストール済み（apt install python3 python3-venv 等）
...
BIN_OUT="$SCRIPT_DIR/bin/linux"
VENV_DIR="$SCRIPT_DIR/build_tmp/venv_linux"
...
"$VENV_PY" -m PyInstaller \
    ...
    --name main \
    --distpath "$BIN_OUT" \
    --workpath "$SCRIPT_DIR/build_tmp/linux" \
...
npx vsce package --no-dependencies --out "ai-audit-linux-${PKG_VERSION}.vsix"

echo ""
echo "Done! Distribute: ai-audit-linux-${PKG_VERSION}.vsix"
